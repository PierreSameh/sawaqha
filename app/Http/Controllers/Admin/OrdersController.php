    ...
    public function approve($id) {
        ...
        if ($order->status === 1) {
            ...
            if ($order->user->user_type == 1) {
                $order->user->expected_profit = $order->user->expected_profit + (float) $order->sub_total - ((float) $order->product->product->wholesale_price * $order->product->quantity);
                $order->user->balance = $order->user->balance + (float) $order->sub_total - ((float) $order->product->product->wholesale_price * $order->product->quantity);
                $order->user->save();
